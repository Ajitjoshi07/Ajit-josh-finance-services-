...
export const gstAPI = {
  ...
  markFiled: (clientId, fy, month) => api.post(`/gst/file/${clientId}/${fy}/${month}`),
  unmarkFiled: (clientId, fy, month) => api.post(`/gst/unfile/${clientId}/${fy}/${month}`),
...
export const bookkeepingAPI = {
  ...
  submitTrialBalance: (d) => api.post('/bookkeeping/trial-balance/submit', d),
...
export const reportsAPI = {
  ...
  icaiComplete: (p) => api.get('/reports/icai-complete', { params: p }),
}
...
export const adminAPI = {
  ...
  updateGstStatus: (cid, fy, m, s) => api.put('/admin/gst-filing/update-status', null, { params: { client_id: cid, financial_year: fy, month: m, status: s } }),
}
...
export const exportAPI = {
  ...
  // Token is fetched fresh at call time - never stale
  transactionsUrl: (fy, clientId) => `${BASE_URL}/export/excel/transactions?financial_year=${fy}&client_id=${clientId}&token=${getToken()}`,
  gstUrl: (fy, clientId) => `${BASE_URL}/export/excel/gst?financial_year=${fy}&client_id=${clientId}&token=${getToken()}`,
  tdsUrl: (fy, clientId) => `${BASE_URL}/export/excel/tds?financial_year=${fy}&client_id=${clientId}&token=${getToken()}`,
  completeUrl: (fy, clientId) => `${BASE_URL}/export/excel/complete?financial_year=${fy}&client_id=${clientId}&token=${getToken()}`,
  download: (url) => { window.location.href = url }, // use window.location for file downloads
}
