...
import {
  Upload, FileText, Trash2, RefreshCw, Download, Eye,
  CheckCircle, AlertCircle, Clock, ArrowLeft, Users, TrendingUp, X
} from 'lucide-react'
...
const DOC_CATEGORIES = [
  ...
  { value: 'sales_return',        label: 'Sales Return / Credit Note',   group: 'Sales / Income',       color: 'bg-blue-100 text-blue-800',     desc: 'Credit notes to customers — reduces output GST' },
  ...
  { value: 'purchase_return',     label: 'Purchase Return / Debit Note', group: 'Purchases / Expenses', color: 'bg-purple-100 text-purple-800', desc: 'Debit notes to suppliers — reduces input GST' },
  ...
  { value: 'cash_book',           label: 'Cash Book / Cash Flow',        group: 'Bank & Finance',       color: 'bg-green-100 text-green-800',   desc: 'Cash transactions — opening/closing balance' },
  ...
  { value: 'eway_bill',           label: 'E-Way Bill',                   group: 'GST Documents',        color: 'bg-indigo-100 text-indigo-800', desc: 'Transport document for goods > ₹50,000' },
...
function StatusBadge({ status }) {
  const map = {
    completed:  { cls: 'badge-success', icon: CheckCircle, label: 'Processed' },
    pending:    { cls: 'badge-warning', icon: Clock,        label: 'Pending OCR' },
    processing: { cls: 'badge-info',    icon: RefreshCw,    label: 'Processing' },
    failed:     { cls: 'badge-danger',  icon: AlertCircle,  label: 'Failed' },
  }
  const { cls, icon: Icon, label } = map[status] || map.pending
  return <span className={`${cls} flex items-center gap-1 text-xs`}><Icon size={10} />{label}</span>
}
    ...
    <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col">
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
          <div>
            ...
            <p className="text-xs text-gray-500 mt-0.5">
              {DOC_CATEGORIES.find(c => c.value === doc._catKey)?.label || doc._catKey} • {doc.processing_status}
            </p>
          </div>
        ...
        <div className="flex-1 overflow-auto p-4 bg-gray-100 min-h-0">
          {isPdf ? (
            <iframe src={viewUrl} className="w-full h-full min-h-[500px] rounded-lg border" title={doc.original_filename} />
          ) : isImage ? (
            <img src={viewUrl} alt={doc.original_filename} className="max-w-full mx-auto rounded-lg shadow" />
          ) : (
        ...
        {doc.extracted_data && Object.keys(doc.extracted_data).some(k => doc.extracted_data[k]) && (
          <div className="px-5 py-3 border-t border-gray-100 bg-gray-50">
            <p className="text-xs font-semibold text-gray-500 mb-2">EXTRACTED DATA</p>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              {[
                ['Invoice No', doc.extracted_data.invoice_number],
                ['Date', doc.extracted_data.invoice_date],
                ['Total Amount', doc.extracted_data.total_amount ? `₹${parseFloat(doc.extracted_data.total_amount).toLocaleString()}` : null],
                ['GSTIN', doc.extracted_data.gstin],
              ].filter(([, v]) => v).map(([k, v]) => (
                <div key={k}>
  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-500 mt-1">Select a client to manage their documents</p>
      </div>
      {isLoading ? (
        ...
        <div className="card p-16 text-center text-gray-400">
          ...
          <p className="text-sm mt-1">Go to Admin → Create New Client first</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {clients.map(c => (
            <button key={c.id} onClick={() => onSelectClient(c)}
              className="card p-5 text-left hover:border-primary-300 hover:shadow-md transition-all cursor-pointer group">
              <div className="flex items-center gap-3 mb-3">
                <div className="w-10 h-10 bg-primary-100 rounded-xl flex items-center justify-center group-hover:bg-primary-600 transition-colors">
                  <Users size={18} className="text-primary-600 group-hover:text-white transition-colors" />
                </div>
              ...
              <div className="grid grid-cols-2 gap-2 text-xs text-gray-500 mb-2">
                <span><b>PAN:</b> {c.pan || '—'}</span>
                <span><b>Type:</b> {c.business_type || '—'}</span>
                <span><b>State:</b> {c.state || '—'}</span>
                <span><b>FY:</b> {c.current_fy}</span>
              </div>
              <p className="text-xs text-primary-600 font-medium">Click to manage documents →</p>
            </button>
          ))}
        </div>
      )}
...
// Document Manager
function DocumentManagerView({ clientId, clientName, onBack }) {
  ...
  const onDrop = useCallback(async (acceptedFiles) => {
    if (!clientId) {
      toast.error('No client profile found. Complete your profile first.')
      return
    }
    setUploading(true)
    let success = 0
    for (const file of acceptedFiles) {
      ...
      // For admin uploads, include client_id
      // For client uploads, backend resolves it automatically from profile
    ...
    setUploading(false)
    if (success > 0) {
      toast.success(`${success} file(s) uploaded — OCR processing started. GST/ITR/reports will update automatically.`)
      refetch()
    }
  }, [docType, fy, clientId])
  ...
  const displayDocs = (activeCategory === 'all'
    ...
    : (catData?.categories?.[activeCategory] || []).map(d => {
        const cat = DOC_CATEGORIES.find(c => c.value === activeCategory)
        return { ...d, _catKey: activeCategory, _catLabel: cat?.label, _catColor: cat?.color }
      })
  )

  const selectedDocType = DOC_CATEGORIES.find(c => c.value === docType)
    ...
    <div className="p-6 space-y-5">
      ...
      <div className="flex items-center gap-3 flex-wrap">
        ...
        <div className="flex-1">
          <h1 className="text-2xl font-bold text-gray-900">{clientName || 'My Documents'}</h1>
          <p className="text-sm text-gray-500">
            {catData?.summary?.total || 0} files • {catData?.summary?.processed || 0} processed • FY {fy}
          </p>
        </div>
      ...
      {catData && (
        <div className="grid grid-cols-3 md:grid-cols-7 gap-2">
          {DOC_GROUPS.map(grp => {
            const count = DOC_CATEGORIES.filter(c => c.group === grp)
              .reduce((s, c) => s + (catData.categories?.[c.value]?.length || 0), 0)
            return (
              <button key={grp} onClick={() => { setActiveGroup(grp); setActiveCategory('all') }}
                className={clsx('card p-3 text-center cursor-pointer transition-all',
                  activeGroup === grp ? 'border-primary-400 bg-primary-50' : 'hover:border-gray-300')}>
                <p className={clsx('text-xl font-bold', count > 0 ? 'text-primary-600' : 'text-gray-300')}>{count}</p>
                <p className="text-xs text-gray-500 leading-tight mt-0.5 line-clamp-2">{grp}</p>
              </button>
            )
          })}
        </div>
      ...
      <div className="card p-5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
          <div>
            ...
            <select className="input" value={docType} onChange={e => setDocType(e.target.value)}>
              {DOC_GROUPS.map(grp => (
                <optgroup key={grp} label={`── ${grp} ──`}>
                  {DOC_CATEGORIES.filter(c => c.group === grp).map(c => (
            ...
            {selectedDocType && (
              <p className="text-xs text-gray-500 mt-1">📌 {selectedDocType.desc}</p>
            )}
          </div>
          <div>
          ...
          <div className="flex flex-col items-center gap-3">
            ...
            <div>
              <p className="font-medium text-gray-700">
                {uploading ? 'Uploading & processing...' : isDragActive ? 'Drop files here' : 'Drag & drop or click to select files'}
              </p>
              <p className="text-sm text-gray-500 mt-1">PDF, JPG, PNG, XLSX — max 20MB each — multiple files at once</p>
              <p className="text-xs text-primary-600 mt-1 font-medium">
                ✓ After upload, OCR extracts data automatically → GST, ITR, P&L update in real-time
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="space-y-2">
        <div className="flex flex-wrap gap-2">
          {['All', ...DOC_GROUPS].map(grp => (
            <button key={grp} onClick={() => { setActiveGroup(grp); setActiveCategory('all') }}
              className={clsx('px-3 py-1.5 rounded-lg text-xs font-medium transition-all',
                activeGroup === grp ? 'bg-primary-600 text-white' : 'bg-white text-gray-600 border border-gray-200 hover:bg-gray-50')}>
              {grp}
            </button>
          ))}
        </div>
        <div className="flex flex-wrap gap-2">
          <button onClick={() => setActiveCategory('all')}
            className={clsx('px-3 py-1.5 rounded-lg text-xs font-medium transition-all',
              activeCategory === 'all' ? 'bg-gray-800 text-white' : 'bg-white text-gray-600 border border-gray-200 hover:bg-gray-50')}>
          ...
          {filteredCats.map(cat => {
            ...
            return (
              <button key={cat.value} onClick={() => setActiveCategory(cat.value)}
                className={clsx('px-3 py-1.5 rounded-lg text-xs font-medium transition-all',
                  activeCategory === cat.value ? 'bg-gray-800 text-white' : 'bg-white text-gray-600 border border-gray-200 hover:bg-gray-50')}>
      ...
      <div className="card">
        <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between flex-wrap gap-2">
          <h3 className="font-semibold text-gray-900">
            {activeCategory === 'all' ? `${activeGroup} Documents` : DOC_CATEGORIES.find(c => c.value === activeCategory)?.label}
            <span className="ml-2 badge-gray">{displayDocs.length}</span>
          </h3>
        </div>
        {isLoading ? (
          <div className="flex justify-center py-16"><div className="spinner w-8 h-8" /></div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-gray-50">
                  {['Filename', 'Category', 'Month', 'Status', 'OCR Score', 'Size', 'Date', 'Actions'].map(h => (
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        ...
                        <div className="min-w-0">
                          <p className="truncate max-w-[150px] font-medium text-gray-800" title={doc.original_filename}>
                            {doc.original_filename}
                          </p>
                          {doc.extracted_data?.invoice_number && (
                            <p className="text-xs text-gray-400">#{doc.extracted_data.invoice_number}</p>
                          )}
                          {doc.extracted_data?.total_amount && (
                            <p className="text-xs text-green-600">₹{parseFloat(doc.extracted_data.total_amount).toLocaleString()}</p>
                          )}
                        </div>
                      </div>
                    </td>
                    <td className="px-4 py-3">
                      <span className={`text-xs px-2 py-1 rounded-full font-medium ${doc._catColor || 'bg-gray-100 text-gray-600'}`}>
                        {doc._catLabel || doc._catKey}
                      </span>
                    </td>
                      ...
                      <div className="flex items-center gap-1.5">
                        <div className="w-12 h-1.5 bg-gray-200 rounded-full overflow-hidden">
                          <div className={clsx('h-full rounded-full',
                            (doc.confidence_score || 0) > 0.7 ? 'bg-green-500' :
                            (doc.confidence_score || 0) > 0.4 ? 'bg-amber-400' : 'bg-red-400')}
                            style={{ width: `${(doc.confidence_score || 0) * 100}%` }} />
                        </div>
                        <span className="text-xs text-gray-500">{Math.round((doc.confidence_score || 0) * 100)}%</span>
                      </div>
                    </td>
                    <td className="px-4 py-3 text-gray-500 text-xs whitespace-nowrap">{fmt(doc.file_size)}</td>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1">
                        ...
                        <button onClick={() => setViewDoc(doc)}
                          className="p-1.5 text-gray-400 hover:text-primary-600 hover:bg-primary-50 rounded" title="View document">
                          <Eye size={14} />
                        </button>
                        {/* Download */}
                        <a href={documentsAPI.downloadUrl(doc.id)} target="_blank" rel="noreferrer"
                          className="p-1.5 text-gray-400 hover:text-green-600 hover:bg-green-50 rounded" title="Download">
                          <Download size={14} />
                        </a>
                        {/* Reprocess */}
                        <button onClick={() => reprocessMutation.mutate(doc.id)}
                          className="p-1.5 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded" title="Reprocess OCR">
                          <RefreshCw size={14} />
                        </button>
                        {/* Delete */}
                        <button onClick={() => { if (window.confirm(`Delete "${doc.original_filename}"?`)) deleteMutation.mutate(doc.id) }}
      ...
      <div className="card p-4 bg-blue-50 border-blue-200">
        <div className="flex items-start gap-3">
          ...
          <div>
            <p className="font-semibold text-blue-900 text-sm">How uploaded documents update your accounts automatically</p>
            <div className="grid grid-cols-2 md:grid-cols-3 gap-2 mt-2 text-xs text-blue-700">
              {[
                ['Sales Invoice', '→ Output GST, GSTR-1, P&L income'],
                ['Purchase Invoice', '→ Input GST (ITC), GSTR-2B, P&L cost'],
                ['TDS Certificate', '→ ITR tax credit, Form 26AS'],
                ['Expense Bill', '→ P&L expenses, bookkeeping'],
                ['Bank Statement', '→ Cash flow, reconciliation'],
                ['Salary Slip', '→ TDS 192, PF/ESIC, wages'],
              ].map(([doc, calc]) => (
                <div key={doc} className="bg-white/60 rounded-lg p-2">
...
export default function DocumentsPage() {
  ...
  // For client users, get their own profile
  ...
  if (!isCA()) {
    ...
    const clientName = myProfile?.profile?.business_name || user?.full_name || 'My Documents'
    return (
      <DocumentManagerView
        clientId={clientId}
  ...
  if (!selectedClient) {
    return <ClientSelectorView onSelectClient={setSelectedClient} />
  }
